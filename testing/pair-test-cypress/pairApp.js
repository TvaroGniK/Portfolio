  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    function valueCard() {
      ...
      function checkCard(event) {
        ...
        if (firstCard !== null && lockCard == false) {
          // даем значение второй карте
          if (secondCard == null) {
            secondCard = event.target;
            lockCard = true;
            if (secondCard == firstCard) {
              // проверка на двойной клик по одной карте
              setTimeout(() => {
                closeCard(); // сборас значений открытых карт
              }, 100);
            } else if (firstCard.dataset.number === secondCard.dataset.number) {
              // сравниваем значение дата атрибутов открытых карт
              openCard();
              countPair++;
        ...
        } else if (lockCard == false) {
          // даем значение первой открытой карте
